"""
BiLSTM (Single-Shot) 기반 6-DOF 시계열 예측 학습 스크립트
...
class SingleShotRNN(nn.Module):
    ...
    FC 한 번(single-shot)으로 미래 OUT_LEN 스텝 전체를 예측.
    ...
    def __init__(self, input_dim=6, output_dim=6, in_len=128, out_len=128,
        ...
        # 마지막 은닉 상태 → 미래 전체(out_len * output_dim)를 한 번에 예측
    ...
    def forward(self, x):
        ...
        pred = self.fc(last_hidden)                              # (B, OUT_LEN*6)
        return pred.view(-1, self.out_len, self.output_dim)     # (B, OUT_LEN, 6)
