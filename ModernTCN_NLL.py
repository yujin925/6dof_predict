...
EXP_NAME = f"CausalModernTCN_NLL_full_train_in{IN_LEN}_out{OUT_LEN}_stride{STRIDE}"
RESULT_DIR = Path("experiments0825") / EXP_NAME
...
class ModernTCNRegressor(nn.Module):
    """평균(mu)과 log 분산(log_var)을 함께 출력하는 확률적 회귀 모델"""
    ...
    def forward(self, src):
        ...
        mu, log_var = x.chunk(2, dim=-1)  # 각각 (B, OUT_LEN, DOF)
